...
- [store.ts](../src/lib/progress/store.ts): Firestore I/O for **gameplay/resume
  state only** — per-step status, current step, and last submission saved to
  `progress/{uid}_{lessonId}` (the learner's own, non-currency data).
- **Spark awards are server-authoritative.** The lifetime balance
  (`totalPoints`), streak, `activeDays`, and `completedLessons` are mutated **only**
  by the `recordStepCompletion` Cloud Function ([functions/src/index.ts](../functions/src/index.ts)),
  which recomputes points from bundled built-in lesson metadata
  ([functions/src/builtinLessonMeta.ts](../functions/src/builtinLessonMeta.ts), generated by
  `npm run gen-lesson-meta`) and records them in a server-only `rewards/{uid}_{lessonId}`
  ledger so each step pays out at most once. The client never writes its own
  balance, so it cannot be forged. [rewards.ts](../src/lib/progress/rewards.ts) is the
  client wrapper that calls the function. Custom AI lessons are practice-only and
  award nothing.
- [useLessonProgress.ts](../src/lib/progress/useLessonProgress.ts): React hook
  wiring the reducers to Firestore (resume state) and the reward callable; used by
  `LessonPage`.
...
- `users/{uid}`: `{ displayName, email, totalPoints, currentStreak,
  ...
  Profile fields are owner-writable; the aggregate/currency fields are server-only.
- `users/{uid}/aiLessons/{id}`: a saved custom lesson — owner-readable, written
  only by the `commitCustomLesson` function (Admin SDK).
- `progress/{uid}_{lessonId}`: a `LessonProgress` doc (gameplay/resume state).
- `rewards/{uid}_{lessonId}`: the server-only Spark ledger.

Rules ([firestore.rules](../firestore.rules)): a user reads only their own
`users/{uid}` doc and may edit profile fields but **never** the balance/streak
aggregates (those change only via the `recordStepCompletion` function, which uses
the Admin SDK to bypass rules); a new profile must start with zeroed aggregates.
`progress` docs are read/written only by the owning uid. `aiLessons` are
owner-readable but client-write-blocked, and `rewards` is fully server-only.
`courses` are world-readable, never client-writable.
