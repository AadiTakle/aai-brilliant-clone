...
- LOCK THE TASK DOWN. A question only proves mastery if its constraints make the INTENDED solution the ONLY shape that passes. Otherwise a learner can hit the expected output with a shortcut — printing a literal, hardcoding the answer, or calling a banned builtin — and "pass" without doing the work. So whenever the prompt implies a particular technique, you MUST set the matching fields below. These fields EXIST for exactly this purpose — use them by default and only omit one when no valid beginner solution could satisfy it. Never disallow something you also require, and never require a name with no place in a valid solution.
  ...
  • "requiredNames": an array of identifiers the solution MUST use. ONLY require a variable name that YOUR PROMPT EXPLICITLY GIVES the learner — e.g. ["total"] only if the prompt literally tells them to keep the running total in a variable called total. If the prompt lets the learner choose the variable name (or never names one), do NOT list it here; enforce the work with "forbidHardcodedOutput"/"requiredConstructs" instead. (Requiring "def" to force defining a function is fine — that is a keyword, not a learner-chosen variable name.)
...
- WORKED EXAMPLE. Prompt: "Store the number 30 in a variable named answer, then print that variable." Because the prompt NAMES the variable (answer), set "requiredNames": ["answer"] AND "forbidHardcodedOutput": true — now a bare print("30") fails and the only passing shape is answer = 30 then print(answer). If the prompt had instead said "store it in a variable of your choosing", you would keep "forbidHardcodedOutput": true but leave "requiredNames" empty, since the learner may name the variable anything. Another: "Add up 1 through 5 and print the total" → "requiredConstructs": ["loop"], "forbidHardcodedOutput": true, and optionally "disallowedNames": ["sum"]; the reference loops over range, accumulates into total, and prints it.
